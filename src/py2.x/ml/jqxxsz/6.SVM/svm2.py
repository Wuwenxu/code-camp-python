...
def testRbf(k1 = 1.3):
    ...
    Returns:
        无
    """
    dataArr,labelArr = loadDataSet('D:/python/AILearners/data/ml/jqxxsz/6.SVM/testSetRBF.txt')                        #加载训练集
    ...
    dataArr,labelArr = loadDataSet('D:/python/AILearners/data/ml/jqxxsz/6.SVM/testSetRBF2.txt')                         #加载测试集
    errorCount = 0
    datMat = np.mat(dataArr); labelMat = np.mat(labelArr).transpose()         
    m,n = np.shape(datMat)
    for i in range(m):
